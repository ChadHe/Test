...
sap.ca.scfld.md.controller.BaseFullscreenController.extend("hcm.mytimesheet.view.S3", {
	...
	updateData: function() {
		...
		var s = this;
		this.oService.getWorkDays(this, this.oApplication.pernr, m.getData().weekStart, m.getData().weekEnd, function(d) {
			s.getTimeSheetCalendar(d);
			if (m.getData().activities) {
				s.setWeeklyData(m.getData().activities);
			}
		});
		this.oService.getTimeDataList(this, this.oApplication.pernr, m.getData().weekStart, m.getData().weekEnd, function(d) {
	...
	setWeeklyData: function(d) {
		...
		for (i = 0; i < p.days.length; i++) {
			...
			for (j = 0; j < p.days[i].entries.length; j++) {
				if (p.days[i].entries[j].timeUnit === "H") {
